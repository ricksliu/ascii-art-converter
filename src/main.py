...
def main():
    with open(util.get_abs_path("../config.json")) as file:
        config = json.load(file)
        INPUT_PATH = glob.glob(util.get_abs_path("../" + config["inputPath"]))
        OUTPUT_PATH = util.get_abs_path("../" + config["outputPath"])
    ...
    for input_path in INPUT_PATH:
        ...
        font = font_util.get_font(util.get_abs_path("consolas.ttf"), ASCII_RES_W, output_width)
